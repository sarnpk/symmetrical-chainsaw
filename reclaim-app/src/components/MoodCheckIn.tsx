    ...
    <div className="space-y-6">
      ...
      {showJourney && aiSuggestions.length > 0 && (
        <Card className="border-indigo-200 bg-indigo-50">
          <CardHeader>
            <CardTitle className="text-indigo-900 text-base flex items-center gap-2"><Lightbulb className="h-4 w-4" />Suggestions for You</CardTitle>
          </CardHeader>
            ...
            {aiSuggestions.map((suggestion, idx) => (
              <div key={idx} className={`p-3 rounded-lg border ${
                ...
                {suggestion.action && (
                  ...
                  >
                    {suggestion.action.label} <ArrowRight className="h-3 w-3" />
                  </a>
                )}
              </div>
            ))}
