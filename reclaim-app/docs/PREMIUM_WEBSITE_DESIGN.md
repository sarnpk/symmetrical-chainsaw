# Reclaim — Premium Website Design & Implementation Plan

**Version:** 1.0 · **Audience:** designers, engineers, PMs · **Build target:** Next.js 14 App Router · Tailwind CSS v4 · React 18 · framer-motion
**Status:** Ready to implement. No guesswork required — every token, component, page, and rule is specified below.
...
| **Relief / hope** | Warm ascent gradient (indigo → violet → rose) evoking "light at the end of the tunnel"; soft serif headlines that read as human, not corporate | Global section treatment, headings |
| **Commitment (Cialdini)** | Free quiz asks for nothing but an email → creates a small commitment → email-prefilled auth funnel | Free tools, auth redirect |
...
**Golden rule:** the marketing site must feel like the calmest, safest room on the internet for this audience — while still converting. If a design choice adds visual noise, it is rejected.
...
- **One-line product story:** Reclaim is a private, AI-assisted journal and evidence platform that helps survivors of emotional and narcissistic abuse document their reality, spot manipulation patterns, and build legally-usable records — all in complete privacy.

### 1.2 Voice & Tone
```
Empathetic      → never clinical; always "you", never "users"
Calm            → present tense, short sentences, no exclamation spam
Credible        → specificity beats hype ("timestamps + GPS + hash-locked exports")
Empowering      → focus on action the survivor controls
Safe            → privacy claims stated plainly wherever data is mentioned
```
**Write examples**
- ✅ "Every entry is encrypted and locked to your account. Your data is never sold or trained on."
- ❌ "Your data is super secure with industry-leading protection!!!"
...
| Color emotion | Calm → hopeful ascent |
...
```css
/* src/styles/tokens.css — paste into globals.css inside @theme (see §11.2) */
@theme {
  /* Brand primaries */
  ...
  /* Neutral warm-gray (softer than pure gray — reduces "clinical" feel) */
  --color-ink-50:   #fafafa;
...
| Token | Usage | Notes |
|---|---|---|
| `brand-600` | Buttons, links, focus rings, active nav, primary fill | Hover → `brand-700`; **never** on large background areas longer than one section |
| `brand-500` | Browser theme-color, small accents, slider | Matches current `#6366f1` so no breakage |
| `hope-500/600` | "Recovery/hope" sections, quiz CTAs, gradient partner | Keep ≤ 20% of a page |
| `dawn-*` | Crisis (🆘), urgent CTAs, error | One widget only (existing `CrisisToolkitWidget`) |
...
### 2.3 Accessibility (WCAG 2.1 AA) — mandates
| Pair | Contrast |
|---|---|
| `brand-600` (#4f46e5) on white | 7.2:1 ✅ (AA/AAA text) |
| white on `brand-600` | 5.0:1 ✅ |
| `brand-600` on `brand-50` | 7.9:1 ✅ |
| `ink-500` on white | 4.6:1 ✅ (muted text ≥ 4.5 for body; use `ink-600` for captions < 18px) |
...
### 3.1 Fonts
- **Headings & display:** `Fraunces` (optical size, SOFT axes) — `next/font/google`. Humanist old-style serif = warmth, editorial authority, emotional resonance. Use `font-variation-settings: "SOFT" 100` for the hero.
- **Body & UI:** `Inter` (already in use) — legibility, neutrality, proven at small sizes.
- **Monospace (evidence/export timestamps):** `JetBrains Mono` for hash/ID displays (adds "forensic" credibility) — optional, subset only.
...
| `button` | 0.9375rem / 1 / 600 | — | Buttons, nav |
| `eyebrow` | 0.75rem / 1.1 / 600, letter-spacing 0.08em, uppercase | — | Section labels |
...
- **Hierarchy delimiter:** no rules/hairlines above or below headings — spacing does the work (`margin-top: 1.5x font-size` rhythm).
...
```
/
├── /pricing                      → 3 tiers + comparison (§5.5)
├── /faq                          → grouped accordions (Safety/Legal/Privacy/Pricing)
├── /blog                         → content hub
│   ├── /blog/[slug]              → dynamic posts (Supabase blog_posts)
├── /learn-more                   → Legal evidence deep-dive (SEO page)
├── /donate                       → Ko-fi give
├── /free-assessment              → lead magnet #1 (quiz)
├── /free-narcissist-test         → lead magnet #2 (quiz)
├── /relationship-health-check    → lead magnet #3 (quiz)
├── /discard-stage-test           → lead magnet #4 (quiz)
├── /gaslighting-reality-check    → lead magnet #5 (quiz)
├── /privacy | /terms | /cookies | /gdpr   → legal
├── /auth | /signin | /forgot-password     → auth
└── /auth/callback                → OAuth handler (route.ts)
```

### 4.2 App surface (unchanged, protected)
`/dashboard`, `/journal`, `/reality-log`, `/belief-reframe`, `/narcissist-detector`, `/narcissist-simulator`, `/safety-plan`, `/community`, `/ai-coach`, `/settings`, `/subscription`, `/admin`, … (full list in codebase)

### 4.3 IA rationale (psychology)
- **Quiz pages at top level but identical visual family** → user recognizes the funnel; every quiz ends in the same email-capture + prefilled `/auth?email=…` handoff. This is the **lead-magnet network effect**: five entry points, one conversion flow.
- **Pricing accessible in 1 click from everywhere** → removes friction for users whose decision driver is cost, not education.
- **Legal/evidence content is second-level (`/learn-more`)** → signals legitimacy without making the marketing homepage feel like a courtroom.

### 4.4 Navigation (primary)
Desktop: `Free Tools ▾ | Pricing | Blog | FAQ | Donate | Sign in | Get Started →`
Mobile: full-screen slide-over menu (□ together with hamburger), sticky bottom "Get Started" bar appears only after scroll past hero (reduces anxiety at first load).
...
Every marketing section = `eyebrow → display-2 → lead(body-lg, max-w-2xl) → content grid → centered CTA`. One visual idea per viewport.
...
| 3 | **Hero** | split layout: left copy (eyebrow → H1 with gradient word → lead → dual CTA → micro trust row), right visual = animated "evidence dashboard" mock + floating mini-cards (parallax). Variant `size="center"` for internal pages. | `src/components/marketing/Hero.tsx` |
| 4 | **TrustBar** | 4 items, separated by dots: "AES-256 encrypted" · "Never sold or trained on" · "70+ languages" · "Export court-ready PDFs". | `src/components/marketing/TrustBar.tsx` |
...
| 12 | **QuizShell** | shared frame for the 5 free tools: progress bar, single-question-at-a-time, no back-button on final, dedicated result → email capture. | `src/components/marketing/QuizShell.tsx` |
| 13 | **Reveal** | IntersectionObserver wrapper: `fade-up`, 8px→0 translate, 300ms, stagger via `delay` prop; disabled under reduced motion. | `src/components/marketing/Reveal.tsx` |
| 14 | **Logo** | SVG wordmark (proposal below) — replaces raster `/logo.png` on marketing surfaces. | `src/components/marketing/Logo.tsx` |
...
| 5 | Free tools grid | FeatureCard ×5 | hover lift + icon colorize | funnel entry points |
| 6 | Featured AI tools | FeatureCard (featured variant) | reveal stagger | depth → premium perception |
| 7 | Features grid ("Evidence-based toolkit") | FeatureCard ×14 | stagger two-by-two | coverage/authority |
...
| 11 | Pricing | PricingCard ×3 + toggle | toggle crossfade; popular card pulse-once | monetization |
...
| 14 | Footer | SiteFooter | — | trust + escape routes (privacy/crisis) |
...
|---|---|---|
| Scroll reveal | `opacity 0→1; translateY(8px→0); 300ms; stagger 80ms` | gradual mastery → reduces overwhelm |
...
| Count-up | 600–1200ms ease-out, digits only | proof lands as "fact" |
...
| Nav bar | 8px scroll → 56px height + shadow | direction signal, reduced chrome |
| Page transitions | none (SPA-style avoided for SEO/perf) | predictable, fast |

**Performance guardrails:** no layout animations (`transform/opacity` only), no heavy blur during scroll (backdrop-blur only on static header), LCP element (hero H1) never animated *into* view with opacity that blocks it — reveal hero before LCP threshold or keep H1 static.
...
| 640–1023 (tablet) | hamburger | stacked, visual under copy | 2-col | 3-col squeezed / 2+1 | full-width steps |
| ≥1024 (desktop) | full nav | 60/40 split | 3–4-col | 3-col, center elevated | centered 720px column |

Mobile specifics:
- Tap targets ≥ 44×44px.
- No horizontal scroll ever (`overflow-x-hidden` on body is banned as a mask — fix real overflow issues in QA).
- Floating crisis widget stays ≤ 56px and never overlaps sticky CTA (place CTA left-bottom, widget right-bottom).
...
| Metric | Target |
|---|---|
| LCP | ≤ 2.0s (hero text static; hero image `priority` + `fetchPriority="high"`) |
| CLS | ≤ 0.05 (fixed aspect ratios on all media; no late-loading layout shifts) |
| INP | ≤ 200ms |
| TBT | ≤ 200ms |
| Bundle | Marketing pages ≤ 120KB gzipped JS total; icons tree-shaken via `lucide-react` named imports (already) |

Techniques: `next/font` (font-display swap + preload latin), `next/image` unoptimized already configured — keep raster assets ≤ 1600px wide/WebP, defer non-critical components via `next/dynamic` (testimonials, footer widget), no third-party scripts on first paint except GA (already conditional).
...
- `metadataBase` set (`https://reclaim.app`) — currently missing (build warning exists).
...
- Internal linking from every quiz result → related tool → blog → pricing (topical authority).
...
1. Add the `@theme` block from §2.1.
2. Replace classes globally, in order of blast-radius, using find/replace on marketing files *only*:
   - `indigo-600` → `brand-600`, `indigo-700` → `brand-700`, `indigo-50` → `brand-50`, `indigo-500` → `brand-500`
   - `purple-600` → `hope-600`, `purple-50` → `hope-50`, etc.
   - `gray-900` → `ink-900`, `gray-700` → `ink-700`, `gray-600` → `ink-600`, `gray-500` → `ink-500`, `gray-50` → `ink-50`, `white/80` borders → `ink-200/60`
...
### 12.1 Funnel
```
Cold visit → Quiz page (0 form) → 5 questions → Result (adds value) 
→ Email capture (name + email, optional) → /auth?email=… (prefilled)
→ try a tool in Foundation → usage limit (429 “upgrade_required”) → /pricing → Stripe
```
Every junction gives value **before** asking. Quiz results are genuinely useful and shareable (`ViralResultShare` already exists → keep).

### 12.2 CTA copy bank (respects voice rules)
- Hero primary: **"Start free assessment"** · secondary: **"Explore free tools"**
- Mid-page (after features): **"See how it works"** → `/learn-more`
- After results: **"Create your secured account"**
- Pricing: **"Start free — upgrade anytime"**
- Final banner: **"Begin today. Reclaim your peace."**
- Crisis (always available): **"🚨 Immediate support"** → safety-plan + hotline links

### 12.3 Trust injections per section
Hero → TrustBar; Features → "Built by trauma-informed practitioners"; Pricing → "Cancel anytime · 7-day money-back on paid plans (verify with product)" ; Footer → SSL + privacy statement + crisis line.
...
- Add `@theme` tokens (§2.1), font stack (§3.1), base heading rules (§3.3), motion-safe base (§8).
- Add `--font-display`; set `metadataBase` in `layout.tsx`; fix hydration-preload warning on `<body>` class (move `preload` class handling to suppress mismatch or remove class — see Investigation note §13.5).
- **Files:** `src/app/globals.css`, `src/app/layout.tsx`, `next.config.ts` (if metadataBase needs env).

### Phase 1 — Component library (1 day)
- Build components from §6 with variants & tests ids.
...
### Phase 2 — Landing page rebuild (1 day)
- Rewrite `src/app/page.tsx` to the §7 blueprint (keep server-component auth redirect to `/dashboard`).
- Wire `SiteHeader/SiteFooter`; remove inline header/footer leftovers.

### Phase 3 — Supporting marketing pages (1–1.5 days)
...
### Phase 4 — Motion & polish (½–1 day)
- Apply `Reveal` wrappers, `StatCounter`, hero parallax; add reduced-motion gate; run performance pass (§10.1).

### Phase 5 — SEO/accessibility audit & QA (1 day)
- `sitemap.xml`, canonical/OG fix, Lighthouse ≥ 90 perf / ≥95 a11y / ≥95 SEO, manual keyboard + screen-reader walkthrough, real-device mobile checks (§9), page weight budget.
...
| `src/components/marketing/*` | new (§6) |
| `public/*` | update favicon/apple-icon tsets + OpenGraph image; add `sitemap.xml` |

### 13.2 Dependency budget (no new runtime deps)
Use existing `framer-motion`, `@headlessui/react`, `lucide-react`. Everything in §6 is achievable with them. `Fraunces`/`JetBrains Mono` come from `next/font/google` (zero runtime cost).

### 13.3 Definition of Done (acceptance)
- `npm run build` green; `npm run lint` green.
- Lighthouse (desktop+mobile): perf ≥ 90, a11y ≥ 95, SEO ≥ 95.
...
- Copy passes the voice gate (§1.2) — grep for '!', 'super', 'amazing'.
...
| Supabase paused (current) | Marketing site must render fully static with zero DB calls on first load — already true for `/` except auth check; keep it that way |
...
1. Body-class hydration mismatch (`preload`) — remove conditional class or the script toggling it; verifies instantly.
2. `metadataBase` warning — set to `https://reclaim.app`.
3. `.env.local` has a paused Supabase project (`gstiokcvqmxiaqzmtzmv`) — restore before login testing (§ pending from session).
4. `manifest.json` is stale ("App", Android-only) — refresh name/icons/scope.

---

## 14. Priority Order for First Paw-print

If timeboxed, implement **Phase 0 → 2 → 12.2 CTA copy**, which alone delivers the visible "premium" shift (type system, tokens, hero, CTA discipline). Phases 1, 3, 4 follow for completeness; Phase 5 gates launch.
